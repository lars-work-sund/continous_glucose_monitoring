...
peak_statistics_sample <- function(by, sample_data, min_frac_summaries){
  ...
  out <- sample_data[, .(
    ...
    `Total peaks` = (totPeaks <- as.numeric(sum(filter_max_missing(peak, tmp_included, min_frac_summaries), na.rm = TRUE))),
    `Total nadir` = (totNadir <- as.numeric(sum(filter_max_missing(nadir, tmp_included, min_frac_summaries), na.rm = TRUE))),
    ...
    `MDGE(-)` = ifelse(is.na(tHypo/totNadir) | is.infinite(tHypo/totNadir), 0, tHypo/totNadir),
    `MDGE(+)` = ifelse(is.na(tHyper/totPeaks) | is.infinite(tHyper/totPeaks), 0, tHyper/totPeaks),
